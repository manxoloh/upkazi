...
$()
		.ready(
				function() {
					...
					if (window_width > 767) {
						if ($('.fixed-plugin .dropdown').hasClass(
								'show-dropdown')) {
					...
					$('.switch-navbar-fixed input').change(
							function() {
								$nav = $('nav.navbar').first();

								if ($nav.hasClass('navbar-fixed')) {
									$nav.removeClass('navbar-fixed').prependTo(
											'.main-panel');
								} else {
									$nav.prependTo('.wrapper').addClass(
											'navbar-fixed');
								}
	...
	initAnimationsArea : function() {
		$('.animationsArea .btn').click(
				function() {
					...
					$parent.find('.animated').removeAttr('class').addClass(
							'animated').addClass(animation_class);

					$parent.siblings('.header').find('.title small').html(
							'class: <code>animated ' + animation_class
									+ '</code>');
				});
	},

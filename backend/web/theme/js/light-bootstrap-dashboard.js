...
var mobile_menu_visible = 0, mobile_menu_initialized = false, toggle_initialized = false, bootstrap_nav_initialized = false, $sidebar, isWindows;
...
lbd = {
		...
		$('#minimizeSidebar').click(
				function() {
					...
					if (lbd.misc.sidebar_mini_active == true) {
						...
						$('.sidebar .collapse').collapse('hide').on(
	...
	initSidebarMenu : debounce(function() {
		...
		if (!mobile_menu_initialized) {
			...
			nav_content = '<ul class="nav nav-mobile-menu">' + nav_content
					+ '</ul>';
	...
	initBootstrapNavbarMenu : debounce(
			function() {
				...
				if (!bootstrap_nav_initialized) {
					$navbar = $('nav').find('.navbar-collapse').first().clone(
							true);
					...
					nav_content = '<ul class="nav nav-mobile-menu">'
							+ nav_content + '</ul>';
